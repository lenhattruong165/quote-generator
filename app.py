"""
Quote Image Generator API v8
Chang'e Aspirant Bot — by vy-lucyfer

v8: Fix portrait fade sâu giống Make it a Quote
"""
...
# Portrait - Giống Make it a Quote
PORTRAIT_W = 800
...
def make_vertical_fade_deep(width: int, height: int):
    """
    Mask fade dọc SÂU giống Make it a Quote:
    - 0-40%: Ảnh rõ (alpha=255)
    - 40-70%: Gradient xám → đen
    - 70-100%: Đen hoàn toàn (alpha=0)
    """
    mask = Image.new("L", (width, height), 255)
    pixels = mask.load()

    fade_start = int(height * 0.40)   # Bắt đầu fade sớm hơn
    fade_end = int(height * 0.75)     # Kết thúc fade, bắt đầu đen hoàn toàn
    ...
    for y in range(height):
        if y < fade_start:
            ...
            # Gradient mượt từ 255 → 0
            p = (y - fade_start) / (fade_end - fade_start)
            # Curve mạnh để đen nhanh hơn
...
def render_portrait(text: str, display_name: str, username: str, avatar_url: str):
    """Render portrait với fade SÂU giống Make it a Quote"""
    # Parse text
    segments = parse_text_with_emoji(text)
    ...
    # Avatar cao hơn để có đủ chỗ fade
    avatar_h = int(PORTRAIT_W * 1.3)  # Tăng chiều cao avatar

    # Tổng chiều cao canvas
    ...
    if av:
        ...
        # Fade SÂU giống Make it a Quote
        mask = make_vertical_fade_deep(PORTRAIT_W, avatar_h)
        av_crop.putalpha(mask)
        canvas.paste(av_crop, (0, 0), av_crop)
...
def index():
    return "Quote Generator API v8 is Running!", 200
...
def health():
    return jsonify({
        ...
        "features": ["emoji", "deep-fade-portrait"]
    })
...
def make_vertical_fade_light(width: int, height: int):
    """
    Fade NHẸ cho avatar - chỉ mờ dần, không đen hoàn toàn
    Nền đen sẽ được vẽ riêng ở dưới
    """
    mask = Image.new("L", (width, height), 255)
    pixels = mask.load()

    # Chỉ fade nhẹ ở dưới cùng của avatar
    ...
    for y in range(height):
        if y < fade_start:
            ...
            p = (y - fade_start) / (fade_end - fade_start)
            alpha = int(255 * (1.0 - p) ** 0.5)  # Curve nhẹ
        for x in range(width):
            pixels[x, y] = alpha
...
def fit_text(segments, max_w, max_h, draw, is_portrait=False, has_server=False):
    ...
    for size in range(FONT_MAX, FONT_MIN - 1, -FONT_STEP):
        ...
        name_size = max(FONT_MIN, int(size * 0.55))
        user_size = max(14, int(size * 0.42))
        server_size = max(12, int(size * 0.38))  # Server name nhỏ hơn
        name_h = int(name_size * 1.3)
        user_h = int(user_size * 1.3)
        server_h = int(server_size * 1.3)

        if is_portrait:
            # Thêm chiều cao cho server name + 2 gạch ngang
            extra_h = name_h + user_h + 120
            if has_server:
                extra_h += server_h + 10
            total_h += extra_h
        else:
...
def render_portrait(text: str, display_name: str, username: str, avatar_url: str, server_name: str = None):
    """Render portrait - nền đen từ dưới gạch ngang + tên server"""
    # Parse text
    segments = parse_text_with_emoji(text)
    ...
    # Avatar cao (không fade sâu, chỉ fade nhẹ ở dưới)
    ...
    if av:
        ...
        # Fade nhẹ ở dưới avatar
        mask = make_vertical_fade_light(PORTRAIT_W, avatar_h)
        av_crop.putalpha(mask)
        canvas.paste(av_crop, (0, 0), av_crop)

    # Text area - bắt đầu từ dưới avatar
    draw = ImageDraw.Draw(canvas)
    y = avatar_h + 20

    # Gạch ngang TRÊN CÙNG của text area (đánh dấu bắt đầu nền đen)
    line_w = int(PORTRAIT_W * 0.5)
    line_x = (PORTRAIT_W - line_w) // 2
    draw.line([(line_x, y), (line_x + line_w, y)], fill=COLOR_LINE, width=2)
    y += 15

    # Server name (nếu có) - NGAY DƯỚI GẠCH NGANG
    if server_name:
        server_text = f"@{server_name}"
        bbox = dummy.textbbox((0, 0), server_text, font=text_info["server_font"])
        server_w = bbox[2] - bbox[0]
